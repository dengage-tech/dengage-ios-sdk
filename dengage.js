(function() {
	...
	function showOnsiteMessage(message) {
		...
		return loadHtmlIfNecessary(message).then((function(content) {
			...
			if (contentType === 'TYPEFORM') {
				...
				if (isBanner) {
					container.className = 'dengage-onsite-banner dn-' + position.toLowerCase(), container.innerHTML = generateBannerHtml(content)
				} else {
					var _window$_Dn_globaL_$u;
		...
		var sourceAndParametersMap = {
			...
			SERVER_SIDE: {
				...
				'dn.master_contact.birth_date': function dnMaster_contactBirth_date() {
					...
					if (!visitorBirthDate) return null;
	...
	function checkValue(currentValue, operator, filterValues, dataType) {
		...
		switch (operator) {
			case 'EQUALS': {
				if (filterValues[0] == currentValue) return true
			}
			break;
			case 'NOT_EQUALS': {
				if (filterValues[0] != currentValue) return true
			...
			break;
			case 'LIKE': {
				if (currentValue.includes(filterValues[0])) return true
			}
			break;
			case 'NOT_LIKE': {
				if (!currentValue.includes(filterValues[0])) return true
			...
			break;
			case 'STARTS_WITH': {
				if (currentValue.startsWith(filterValues[0])) return true
			}
			break;
			...
			break;
			case 'ENDS_WITH': {
				if (currentValue.endsWith(filterValues[0])) return true
			}
			break;
			case 'NOT_ENDS_WITH': {
				if (!currentValue.endsWith(filterValues[0])) return true
			...
			case 'NOT_IN':
				...
				break;
			case 'GREATER_THAN': {
				if (currentValue > filterValues[0]) return true
			}
			break;
			case 'GREATER_EQUAL': {
				if (currentValue >= filterValues[0]) return true
